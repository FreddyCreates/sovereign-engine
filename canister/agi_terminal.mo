...
actor AGITerminal {
  ...
  stable var stake           : Nat   = 0;           // FORMA (8 decimals)
  stable var maturity        : Nat   = 0;           // FORMA (8 decimals)
  stable var voteWeight      : Nat   = 0;
  stable var parallaxTreasury: Nat   = 0;           // FORMA
  stable var formaTotal      : Nat   = 0;           // FORMA
  ...
  public func systemTick() : async () {
    ...
    if (tick % SOLVER_EVERY == 0) {
      ...
      formaTotal     := parallaxTreasury + circulating;
    };
  };
  ...
  public shared(msg) func seedEconomics(
    ...
    formaTotal      := _treasury + _circulating;
  };
